...
public:
        ...
        _c(GPIOC)
    {
        ...
        _gpio_set(GPIOA, ~(
                        (chgled ? M(0) : 0UL)
#ifdef PWRDEBUG // не трогаем: usb-пины, если требуется отладка
                        | M(11) | M(12)
#endif
            ));
    ...
    void spion() {
        *SPI1 = _ssave.spi;
        // нам нужно возобновить работу пинов SPI и всех CS
        _gpio_set(GPIOA, M(4) | M(5) | M(6) | M(7) | M(8),
            _a.mode, _a.type, _a.speed, _a.pupd
        );
        // на всякий случай, возобновим пин bmp280-CS, хотя это, вроде как необязательно,
        // т.к. временно включаем SPI мы только для снятия показаний с него
        _gpio_set(GPIOB, M(10),
            _b.mode, _b.type, _b.speed, _b.pupd
        );
        // эта задержка нужна, чтобы успело разойтись питание по sd-карте,
        // которое там появляется с sdcard-cs пина. 
        // без этой задержки, если сразу читать bmp280 при вставленной sd-карте,
        // та начинает шуметь по spi-шине, сбивая показания давления
        for (volatile int i = 0; i < 500; i++)
            // примерно 2мс при базовой частоте 16МГц
            asm("");
    }
    void spioff() {
        *SPI1 = _snull.spi;
...
/*
    ...
    - если не уводить bmp280 в сон (в любом режиме измерения), а только stm: 650 мкА.

    _____________________________________________
    В спящем режиме stm32 часы продолжают работать от внешнего кварца
*/
